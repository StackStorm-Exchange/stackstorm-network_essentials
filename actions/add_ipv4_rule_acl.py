...
class Add_Ipv4_Rule_Acl(DeviceAction):
    def run(self, mgmt_ip, username, password, acl_name, seq_id,
        ...
        seq = []
        output = {}
        seq_variables_std = ('seq_id', 'action', 'src_host_any_ip', 'src_host_ip', 'src_mask',
                             'count', 'log')
        seq_variables_ext = ('seq_id', 'action', 'protocol_type', 'src_host_any_ip', 'src_host_ip',
                             'src_mask', 'src_port',
                             'src_port_number_eq_neq_tcp', 'src_port_number_lt_tcp',
                             'src_port_number_gt_tcp', 'src_port_number_eq_neq_udp',
                             'src_port_number_lt_udp', 'src_port_number_gt_udp',
                             'src_port_number_range_lower_tcp', 'src_port_number_range_lower_udp',
                             'src_port_number_range_higher_tcp', 'src_port_number_range_higher_udp',
                             'dst_host_any_ip', 'dst_host_ip', 'dst_mask', 'dst_port',
                             'dst_port_number_eq_neq_tcp', 'dst_port_number_lt_tcp',
                             'dst_port_number_gt_tcp', 'dst_port_number_eq_neq_udp',
                             'dst_port_number_lt_udp', 'dst_port_number_gt_udp',
                             'dst_port_number_range_lower_tcp',
    ...
    def _parse_(self, protocol_type, statement, key):
        ...
        if statement_list[0] in ['lt', 'gt', 'eq', 'range', 'neq']:
            ...
            if not port == 'range':
                try:
                    output[key + '_port_number_' + port + '_' + protocol_type] = \
                        statement_list.pop(0)
                except:
                    msg = '{} port number {} missing'.format(key, port)
            else:
                try:
                    output[key + '_port_number_' + port + '_lower_' + protocol_type] = \
                        statement_list.pop(0)
                    output[key + '_port_number_' + port + '_higher_' + protocol_type] = \
                        statement_list.pop(0)
                except:
