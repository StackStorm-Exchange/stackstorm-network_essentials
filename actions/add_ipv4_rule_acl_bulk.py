...
class Add_Ipv4_Rule_Acl_Bulk(NosDeviceAction):
    ...
    def switch_operation(self, acl_name, acl_rules):
        ...
        with self.pmgr(conn=self.conn, auth=self.auth, connection_type='NETCONF') as device:
            ...
            if address_type is not 'ip':
                raise ValueError('ACL not compatible for adding IPV4 acl rule')
            ...
            return self._add_ipv4_acl_rules(device,
                                            acl_name=acl_name,
                                            acl_type=acl_type,
    ...
    def _add_ipv4_acl_rules(self, device, acl_name, acl_type, address_type, seqs_list):
        result = {}
