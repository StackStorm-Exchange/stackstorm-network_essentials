...
class ConfigureLogicalInterface(NosDeviceAction):
    """
       ...
       This action achieves the below functionality
           ...
           2.Enable untag/tag vlan on the logical interface
    """
    ...
    def switch_operation(self, logical_interface_number, vlan_type,
        ...
        with self.pmgr(conn=self.conn, auth=self.auth) as device:
            ...
            if device.os_type == 'nos':
                self.logger.error('Operation not supported on this device')
                return

            if vlan_type == 'double_tagged' and inner_vlan_id is None:
            ...
            if vlan_type == 'double_tagged':
                vlan_type = 'tagged'
            pat1 = '\d+r'
            if vlan_type == 'untagged' and re.match(pat1, device.firmware_version) and\
                    vlan_id is None:
            ...
            changes['valid_lif'], lif_list = self._check_interface_presence(device, intf_type,
                                                                            intf_name, lif_name)
            if vlan_type == 'tagged' and vlan_id and inner_vlan_id:
                changes['valid_vlan'], conf_list, dual = self._check_inner_vlan_id(device,
                                                                                   intf_type,
                                                                                   intf_name,
                                                                                   lif_name,
                                                                                   vlan_id,
                                                                                   inner_vlan_id)
            elif vlan_type == 'tagged' and vlan_id:
                changes['valid_vlan'], conf_list, dual = self._check_vlan_id(device, intf_type,
                                                                             intf_name,
                                                                             lif_name, vlan_id)
            ...
            if changes['valid_lif']:
                ...
                if changes['lif_create'] and changes['valid_vlan']:
                    if conf_list != [] and vlan_type == 'tagged' and dual:
    ...
    def _check_vlan_id(self, device, intf_type, intf_name, lif_name, vlan_id):
        ...
        for lif_name, vlan_id in zip(lif_name, vlan_id):
            ...
            if dut_vlan['outer_vlan'] == str(vlan_id):
                ...
                tmp_vlan.remove(vlan_id)
                tmp_lif.remove(lif_name)
        return True, zip(tmp_lif, tmp_vlan), False
    ...
    def _check_inner_vlan_id(self, device, intf_type, intf_name, lif_name, vlan_id, inner_vlan_id):
        ...
        for lif_name, vlan_id, inner_vlan_id in zip(lif_name, vlan_id, inner_vlan_id):
            ...
            if dut_vlan['outer_vlan'] == str(vlan_id):
                ...
                tmp_lif.remove(lif_name)
                tmp_in_vlan.remove(inner_vlan_id)
        return True, zip(tmp_lif, tmp_vlan, tmp_in_vlan), True
