...
class Firmware(NosDeviceAction):
    ...
    def run(self, mgmt_ip, username, password, host_ip, protocol_type,
            proto_username, proto_password, firmware_path):
    ...
    @log_exceptions
    def switch_operation(self, host_ip, protocol_type, proto_username, proto_password, firmware_path):
        with self.pmgr(conn=self.conn, auth=self.auth) as device:
            self.logger.info('successfully connected to %s to download firmware', self.host)
            fwdl_status_dict = device.firmware.download_firmware(protocol=protocol_type, host=host_ip,
                                                                 user_name=proto_username,
                                                                 password=proto_password,
                                                                 directory=firmware_path)
            self.logger.info("Download Status code: %d Status message:%s", fwdl_status_dict['status_code'],
                             fwdl_status_dict['status_message'])
            if fwdl_status_dict['status_code'] == 0:
                """
                Start Monitor process
                """
                self.last_proc_fwdl_entry = 0
                self.fwdl_monitor_timer = Timer(5, lambda: self.firmware_download_monitor_periodic())
