...
class set_l3_system_mtu(NosDeviceAction):
    def run(self, mgmt_ip, username, password, mtu_size,afi):
        ...
        with self.pmgr(conn=self.conn, auth=self.auth) as device:
            self.logger.info('successfully connected to %s to set system IP mtu', self.host)


            changes = self._set_l3_system_mtu(device,
                                              mtu_size=mtu_size,ip_version=ip_version)
            output['result'] = changes
            self.logger.info('closing connection to %s after configuring system IP mtu --'
                             ' all done!', self.host)
        return output
    ...
    def _set_l3_system_mtu(self, device, mtu_size,ip_version):
        ...
        try:
            set_mtu = device.system.system_ip_mtu(mtu=mtu_size,version=ip_version)

            self.logger.info('Successfully  set system IP mtu_size %i on the device',
                                 mtu_size)
