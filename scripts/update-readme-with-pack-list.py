...
PACK_ICON_URL = 'https://raw.githubusercontent.com/StackStorm/st2contrib/master/packs/%(' \
                'name)s/icon.png'
NO_PACK_ICON_URL = 'https://raw.githubusercontent.com/StackStorm/st2contrib/master/packs/st2/icon' \
                   '.png'
...
def generate_pack_list_table(packs):
    ...
    for pack_name, metadata in packs:
        ...
        line = '[![%(name)s icon](%(icon_url)s)](%(base_packs_url)s/%(name)s) | [%(name)s](%(' \
               'base_packs_url)s/%(name)s) | %(description)s | %(keywords)s | [%(author)s](' \
               'mailto:%(email)s) | %(version)s | [click](%(base_repo_url)s#%(name)s-pack)' % (
                   values)
        lines.append(line)
